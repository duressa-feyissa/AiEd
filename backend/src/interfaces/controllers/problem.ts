...
export default function ProblemController(
    ...
    const fetchProblemById = (req: Request, res: Response, next: NextFunction) => {
    ...
    const fetchAllProblems = (
      ...
      const createNewProblem = (req: Request, res: Response, next: NextFunction) => {
        createProblem(req.body, dbRepository)
          .then((problem) => res.json(problem))
          .catch((error) => next(error))
      }

      const updateExistingProblem = (req: Request, res: Response, next: NextFunction) => {

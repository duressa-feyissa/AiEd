...
export default function SubmissionController(
    ...
    const fetchSubmissionById = (req: Request, res: Response, next: NextFunction) => {
        findById(req.params.id, dbRepository)
            .then((submission) => res.json(submission))
            .catch((error) => next(error))
    }

      const createNewSubmission = (req: Request, res: Response, next: NextFunction) => {
        createSubmission(req.body, dbRepository)
          .then((submission) => res.json(submission))
          .catch((error) => next(error))
      }

      const deleteSubmission = (req: Request, res: Response, next: NextFunction) => {
